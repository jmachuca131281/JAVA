...
if [ -z "$JDK" ] && [ -s "$HOME/.IdeaIC2019.2/config/idea.jdk" ]; then
  USER_JRE=$("$CAT" "$HOME/.IdeaIC2019.2/config/idea.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IdeaIC2019.2/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IdeaIC2019.2/config/idea$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=IdeaIC2019.2 \
